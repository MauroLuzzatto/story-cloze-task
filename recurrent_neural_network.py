...
#                print('mse', mse)
    ...
    def classification(self, X_true, X_false):
        ...
        self.X = np.concatenate((X_true, X_false), axis = 0)
        
        # class according to the order of the sentences
        self.y = np.concatenate((np.ones((X_true.shape[0],1)), 
                                 np.ones((X_false.shape[0],1))*2),
                                 axis = 0)

        

        X_train, X_test, y_train, y_test = train_test_split(self.X, self.y, test_size=0.1, random_state = 4, shuffle = True)    
        ...
        self.gridsearch = GridSearchCV(estimator=pipe_lr, param_grid=param_grid, scoring='accuracy', cv=10)
        self.gridsearch = self.gridsearch.fit(X_train, y_train.ravel())
        
        
        
        print("")
        print("Logistic regression: 10-fold cross-validation")
        print("Best score: ", self.gridsearch.best_score_)
        print("Best C:", self.gridsearch.best_params_)
        
        scores = cross_val_score(self.gridsearch, X_test, y_test.ravel(), scoring='accuracy', cv=10)
        ...
        self.prediction_classification = self.doPrediction(self.gridsearch, X_true, X_false)
        
#        print(self.prediction_classification)
        score = float(self.prediction_classification.count(1))/len(self.prediction_classification)
        print('final score', score)
        
#        #test new score
#        score_tt_true = self.gridsearch.predict(X_true).tolist()
#        score_tt = float(score_tt_true.count(1))/len(score_tt_true)
#        print('final score tt', score_tt)
    ...
    def createSubmissionFile(self, X_test_sent_1, X_test_sent_2):
        ...
        final_prediction = self.doPrediction(clf, X_test_sent_1, X_test_sent_2)
    ...
    def doPrediction(self, clf, X_test_sent_1, X_test_sent_2):
